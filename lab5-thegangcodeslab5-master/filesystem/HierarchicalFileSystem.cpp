...
int HierarchicalFileSystem::deleteFile(std::string path) {
	...
	*/
	...
	if (dynamic_cast<DirectoryFile*>(fileToDelete) && (fileToDelete->getSize() > 0)) {//this directory is not empty you cannot remove
		cout << "rm:" << path << ": is a directory that is not empty" << endl;
		 
		return cantdeletedirectory;
